...
def print_activity_table(day,activity_table, activity_table_minutes):
    ...
    print(day + ";" + str(activity_table_minutes['Still']) + ';' + str( activity_table_minutes['Tilting']) + ';' + str(activity_table_minutes['Walking']) + ';' +
...
def activity_tables_for_24hours(day):
    activity_table = {}

    with open('Hourly_statistic.txt', 'r') as reader:
        for line in reader:
            # print(line)
            line_arr = line.split(';')
...
def create_data_hourly(day:str):
    ...
    FMT = '%H:%M:%S'
#    prev_missing_time = datetime.datetime.strptime('00:00:00', FMT)
    prev_missing_time = '00:00:00'


    with open('PhysicalActivity.txt', 'r') as reader, open('Hourly_statistic.txt', 'w') as writer:
        for line in reader:
            # print(line)
            line_arr = line.split(';')
            most_detected_activity = 'None'
            ...
            if line_arr[0] == day and most_detected_activity != 'Unknown activity' and not line_arr[
                ...
                if current_hour != prev_hour:
                    print('difference between hours : ')
                    diff_between_hour = current_hour - prev_hour
                    print(diff_between_hour)
                    ...
                    if diff_between_hour > 1:
                        ...
                        for i in range(diff_between_hour - 1):
                            ...
                            diff = datetime.datetime.strptime(date1, datetimeFormat) - datetime.datetime.strptime(date2,
                                                                                                                  datetimeFormat)
                            print('diffff')
                            diff_arr = diff.__str__().split(':')
                            print(diff_arr)
                            line_arr[1] = diff_arr[0] + ":" + diff_arr[1] + ":" + diff_arr[2]
                            print(line_arr[1])
                ...
                if not smaller_than and check_first_activity == True:
                    print('enter')
                    # if time_comparison(total_time_table, prev_missing_time):
                    #     total_time_table = total_time_table - prev_missing_time
                    print(total_time_table)
                    print(activity_table)
                    print(prev_activity_table)
                    check_first_activity = False
                    exceed_time = True
                    ...
                    print(prev_activity_table)
                    print('time diff prob')
                    print(time_diff)
                    print(total_time_table)
                    # time_diff = total_time_table - time_diff
                    print(time_diff)
    ...
    print(activity_table)
    with open('Hourly_statistic.txt', 'a') as writer:
        prev_date_arr = prev_date.__str__().split(' ')
        prev_date_rev_arr = prev_date_arr[0].split('-')
        prev_date_revert = prev_date_rev_arr[2] + '-' + prev_date_rev_arr[1] + '-' + prev_date_rev_arr[0]
    ...
    print('-------')
    print(all_hours_activity)
    print(len(all_hours_activity))
...
if __name__ == '__main__':
    ...
    day = '10-03-2021'
    activity_tables_for_24hours(day)
    days_of_logger()
